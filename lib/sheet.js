...
const getValue = (target, prop) => {
  ...
  return target.emit(EVENT_NAMES.IDENTIFIER, target, prop);
};
...
class Sheet extends EventEmitter {
  ...
  emit(eventName, ...args) {
    if (eventName === EVENT_NAMES.IDENTIFIER) {
      return this.#handleUnknownIdentifier(eventName, ...args);
    }
  ...
  on(eventName, listener) {
    if (
      eventName === EVENT_NAMES.IDENTIFIER &&
      this.listenerCount(EVENT_NAMES.IDENTIFIER) === 1
    ) {
      this.removeAllListeners(EVENT_NAMES.IDENTIFIER);
    }

    super.on(eventName, listener);
  }

  #handleUnknownIdentifier(eventName, ...args) {
    const [fn = () => undefined] = this.listeners(eventName);
    return fn(...args);
  }
}
